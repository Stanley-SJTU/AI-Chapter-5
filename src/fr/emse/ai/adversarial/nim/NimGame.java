...
public class NimGame implements Game<List<Integer>, Integer, Integer> {
    ...
    public NimGame(int size) {
        initialState.add(0);  // The first number in the List signifies which player
    ...
    public List<Integer> getActions(List<Integer> state) {
        ...
        for (int i = 1; i <= Math.min(3, state.get(1)); i++)  // Upper bound imposed on the number of objects that can be removed in a turn
            actions.add(i);
        return actions;
    }
    ...
    public List<Integer> getResult(List<Integer> state, Integer action) {
        ...
        newState.add(1 - state.get(0));  // Change the player's turn
    ...
    public double getUtility(List<Integer> state, Integer player) {
        if (state.get(0) == 1 - player) {                      // If it is the opponent's turn
            if (state.get(1) == 1)
